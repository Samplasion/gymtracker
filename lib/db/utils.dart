...
List<WorkoutExercisable> databaseRoutineExercisesToExercises(
    List<RoutineExercise> data) {
  final sortOrders = <String, int>{
    for (final e in data) e.id: e.sortOrder,
  };
  final exercises = data
      .map((e) => (e, exerciseFromDatabaseRoutineExercise(e), e.isInSuperset))
      .toList();
  final mapped = {
...
List<WorkoutExercisable> databaseHistoryWorkoutExercisesToExercises(
  ...
  final exercises = data
      .map((e) =>
          (e, exerciseFromDatabaseHistoryWorkoutExercise(e), e.isInSuperset))
      .toList();
  final mapped = {
    for (var (_, exercise, isInSuperset) in exercises)
      if (!isInSuperset) exercise.id: exercise,
...
WorkoutExercisable exerciseFromDatabaseRoutineExercise(RoutineExercise data) {
  assert(() {
    if (!data.isSuperset) {
  ...
  if (data.isSuperset) {
    ...
    return Exercise.raw(
      ...
      parentID: !data.isCustom ? data.libraryExerciseId : data.customExerciseId,
...
WorkoutExercisable exerciseFromDatabaseHistoryWorkoutExercise(
